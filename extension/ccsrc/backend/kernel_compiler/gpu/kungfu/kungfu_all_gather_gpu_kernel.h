...
{
template <typename T>
class KungFuAllGatherGpuKernel : public GpuKernel
{
  public:
    ...
    ~KungFuAllGatherGpuKernel() override
    {
        DestroyResource();
    }
    ...
    {
        LOG_Kernel_Launch("KungFuAllGatherGpuKernel", inputs, workspace,
                          outputs);
        KUNGFU_PROFILE_SITE(KungFuAllGatherGpuKernel::Launch);
        ...
        if (kungfu_use_nccl_scheduler) {
            nccl_scheduler_->Do(
                [=] { KF_LOG_CALL(nccl_controller_->AllGather(w, stream)); });
        } else {
            nccl_controller_->AllGather(w, stream);
        }
        return true;
    }
